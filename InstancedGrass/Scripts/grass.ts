...
interface IBuffer
{
    ...
    /* Offset buffer to store all the other positions */
...
class Grass 
{
    /* All positions of the grass */
    public static AllPositions: Float32Array;

    /* Number of all grass */
    public static NumOfGrass: number;
     ...
     * @param MeshToUse {Mesh}: Mesh to use for rendering
     */
    constructor(VertexShaderSource: string, FragmentShaderSource: string, DiffuseTextureLocation: string, MeshToUse: Mesh)
    {
        this.VertexShaderSource = VertexShaderSource;
...
let MyPosArr: Array<number> = [];

for(let Counter = 0; Counter < 100000; Counter++)
{
    const xPos: number = -200.0 * Math.random() + 100.0;

    MyPosArr.push(xPos);

    MyPosArr.push(0);

    const zPos: number = -200.0 * Math.random() + 100.0;
...
let MyGrass = new Grass(SimpleVertexShader, SimpleFragmentShader, "../Assets/Textures/grass.png", MyQuadMesh);
