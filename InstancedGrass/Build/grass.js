...
var FragmentShaderSource = "#version 300 es\n" +
    ...
    "vec3 PointLightPosition = vec3(0, 30, 0);\n" +
    ...
    "Color = vec4(FinalColor * 2.5, 1.0);\n" +
    "}\n";
var SimpleVertexShader = "#version 300 es\n" +
    "layout (location = 0) in highp vec3 aPosition;\n" +
    "layout (location = 1) in lowp vec2 aTexCoords;\n" +
    "layout (location = 2) in highp vec3 aOffset;\n" +
    "uniform mediump mat4 uProjection;\n" +
    "uniform mediump mat4 uModelView;\n" +
    "out lowp vec2 vTexCoords;\n" +
    "void main(void){\n" +
    "gl_Position = uProjection * uModelView * vec4(aPosition + aOffset, 1.0);\n" +
    "vTexCoords = aTexCoords;\n" +
    "}\n";
var SimpleFragmentShader = "#version 300 es\n" +
    ...
    "out vec4 Color;\n" +
    "void main(void){\n" +
    "vec4 NewColor = vec4(texture(uDiffuseTexture, vec2(vTexCoords.x, 1.0 - vTexCoords.y)));\n" +
    "if(NewColor.a < 0.1) discard;\n" +
    "Color = NewColor;\n" +
    "}\n";
/**************************************************************************/
/********************************** INIT **********************************/
...
var Quad = (function () {
    ...
    Quad.prototype.InitBuffers = function () {
        ...
        this.QuadBuffer.OffsetBuffer = GL.createBuffer();
...
var Grass = (function () {
     ...
     * @param MeshToUse {Mesh}: Mesh to use for rendering
     */
    function Grass(VertexShaderSource, FragmentShaderSource, DiffuseTextureLocation, MeshToUse) {
        this.VertexShaderSource = VertexShaderSource;
        this.FragmentShaderSource = FragmentShaderSource;
        this.DiffuseTextureLocation = DiffuseTextureLocation;
        this.ShaderProgram = CompileShaders(GL, this.VertexShaderSource, this.FragmentShaderSource);
     ...
     * @returns {void}
     */
    Grass.prototype.InitTextures = function (TextureToInit, TextureLocation) {
        TextureToInit.TextureSource = TextureLocation;
        TextureToInit.TextureGL = GL.createTexture();
        TextureToInit.TextureImage = new Image();
        var QuadObject = this;
     ...
     * @returns {void}
     */
    Grass.prototype.ProcessTexture = function (TextureToProcess) {
        GL.bindTexture(GL.TEXTURE_2D, TextureToProcess.TextureGL);
        GL.texParameteri(GL.TEXTURE_2D, GL.TEXTURE_WRAP_S, GL.CLAMP_TO_EDGE);
        GL.texParameteri(GL.TEXTURE_2D, GL.TEXTURE_WRAP_T, GL.CLAMP_TO_EDGE);
        GL.texParameteri(GL.TEXTURE_2D, GL.TEXTURE_MAG_FILTER, GL.LINEAR);
     ...
     * @returns {void}
     */
    Grass.prototype.InitBuffers = function () {
        this.QuadBuffer = {};
        this.QuadBuffer.VertexBuffer = GL.createBuffer();
        this.QuadBuffer.IndexBuffer = GL.createBuffer();
        this.QuadBuffer.TexBuffer = GL.createBuffer();
     ...
     * @returns {void}
     */
    Grass.prototype.InitMatrices = function () {
        this.Matrices = {};
        this.Matrices.ModelViewMat = Mat4.Create();
        this.Matrices.ModelMat = Mat4.Create();
    };
     ...
     * @returns {void}
     */
    Grass.prototype.InitLocations = function () {
        this.Locations = {};
        this.Locations.uModelViewLocation = GL.getUniformLocation(this.ShaderProgram, "uModelView");
        this.Locations.uProjectionLocation = GL.getUniformLocation(this.ShaderProgram, "uProjection");
        this.Locations.uDiffuseTextureLocation = GL.getUniformLocation(this.ShaderProgram, "uDiffuseTexture");
     ...
     * @returns {void}
     */
    Grass.prototype.ProcessBuffers = function () {
        GL.bindBuffer(GL.ARRAY_BUFFER, this.QuadBuffer.VertexBuffer);
        GL.bufferData(GL.ARRAY_BUFFER, this.QuadMesh.Vertices, GL.STATIC_DRAW);
        GL.bindBuffer(GL.ARRAY_BUFFER, null);
        GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, this.QuadBuffer.IndexBuffer);
        ...
        GL.bufferData(GL.ARRAY_BUFFER, this.QuadMesh.TextureCoordinates, GL.STATIC_DRAW);
        GL.bindBuffer(GL.ARRAY_BUFFER, null);
        GL.bindBuffer(GL.ARRAY_BUFFER, this.QuadBuffer.OffsetBuffer);
        GL.bufferData(GL.ARRAY_BUFFER, Grass.AllPositions, GL.STATIC_DRAW);
        GL.bindBuffer(GL.ARRAY_BUFFER, null);
    };
    /**
     * Processes the vertices
     ...
     * @returns {void}
     */
    Grass.prototype.ProcessVertices = function () {
        GL.useProgram(this.ShaderProgram);
        GL.bindVertexArray(this.VAO);
        GL.bindBuffer(GL.ARRAY_BUFFER, this.QuadBuffer.VertexBuffer);
        GL.enableVertexAttribArray(0);
    ...
    Grass.prototype.Render = function () {
        ...
        GL.uniformMatrix4fv(this.Locations.uModelViewLocation, false, this.Matrices.ModelViewMat);
        GL.bindVertexArray(this.VAO);
        GL.drawElementsInstanced(GL.TRIANGLES, this.QuadMesh.NumOfIndices, GL.UNSIGNED_SHORT, 0, Grass.NumOfGrass);
        GL.bindVertexArray(null);
        GL.activeTexture(GL.TEXTURE0);
        GL.useProgram(null);
    };
...
for (var Counter = 0; Counter < 100000; Counter++) {
    var xPos = -200.0 * Math.random() + 100.0;
    MyPosArr.push(xPos);
    MyPosArr.push(0);
    var zPos = -200.0 * Math.random() + 100.0;
    MyPosArr.push(zPos);
}
Grass.AllPositions = new Float32Array(MyPosArr);
Grass.NumOfGrass = Grass.AllPositions.length / 3;
var MyGrass = new Grass(SimpleVertexShader, SimpleFragmentShader, "../Assets/Textures/grass.png", MyQuadMesh);
...
function Init() {
    ...
    MyGrass.Transform = {};
    MyGrass.Transform.Position = Vec3.FromValues(0.0, 0.9, 0.0);
    MyGrass.Transform.Rotation = Quat.Create();
    MyGrass.Transform.Scaling = Vec3.FromValues(1, 1, 1);
    Floor.Transform = {};
    Floor.Transform.Position = Vec3.FromValues(0.0, 0.0, 0.0);
    Floor.Transform.Rotation = Quat.Create();
    Floor.Transform.Scaling = Vec3.FromValues(100, 100, 0.1);
